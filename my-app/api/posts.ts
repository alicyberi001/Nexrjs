...
type fetchPostByUserType = (_: number) => Promise<IFetchPostsResDto>;
export const fetchPostByUser: fetchPostByUserType = async (id) => {
  const client = generateClient();
  const response = await client.get<IFetchPostsResDto>(urls.posts.byUser(id));
  return response.data;
};
